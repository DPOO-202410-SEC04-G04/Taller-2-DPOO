...
public class SandboxListas
{
    ...
    {
    	List<Integer> copyList = new ArrayList<>(listaEnteros);
        return copyList;
    }
    ...
    {
    	List<String> copyList = new ArrayList<>(listaCadenas);
        return copyList;
    }
    ...
    {
    	int[] integerArray = new int[listaEnteros.size()];
        for (int i = 0; i < listaEnteros.size(); i++) {
            integerArray[i] = listaEnteros.get(i);
        }
        return integerArray;
    }
    ...
    public void eliminarEntero(int valor) {
    	// Se utiliza el método removeIf() de la lista, que acepta un predicado como argumento.
        // El predicado aquí es una expresión lambda que compara cada elemento de la lista con el valor dado.
        // Si un elemento es igual al valor dado, se elimina de la lista. - Se utilizo ChatGPT
        listaEnteros.removeIf(n -> n.equals(valor));
    }
     ...
     * @param posicion La posición donde debe quedar el nuevo valor en la lista aumentada.
     *                 Si la posición es menor a 0, se inserta el valor en la primera posición.
     *                 Si la posición es mayor que el tamaño de la lista, se inserta el valor en la última posición.
     */
    ...
    public int contarEnterosRepetidos() {
     ...
     // Verifica si el número no ha sido verificado previamente y si aparece más de una vez en la lista
        for (int num : listaEnteros) {
            if (!checked.contains(num) && Collections.frequency(listaEnteros, num) > 1) {
            	// Si cumple ambas condiciones, incrementa el contador y agrega el número a la lista de verificados - Se utilizo ChatGPT
                count++;
                checked.add(num);
            }
        }
    ...
    public void generarEnteros(int cantidad, int minimo, int maximo) {
        ...
        for (int i = 0; i < cantidad; i++) {
            int randomNumber = (int) (Math.random() * (maximo - minimo + 1)) + minimo;
            listaEnteros.add(randomNumber);
        }
    }
}
